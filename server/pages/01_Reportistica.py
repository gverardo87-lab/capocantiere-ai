# file: server/pages/01_Reportistica.py (Versione 35.0 - DARK MODE SAFE)
...
# Questo blocco rende le tabelle HTML leggibili in entrambi i temi
st.markdown("""
<style>
    .audit-table {
        width: 100%;
    ...
    .text-green { color: #2ecc71; } /* Verde leggibile su dark e light */
    .text-red { color: #e74c3c; }   /* Rosso leggibile su dark e light */
    .text-muted { opacity: 0.7; font-size: 0.9em; }
</style>
""", unsafe_allow_html=True)

# --- HELPER STILI (DATAFRAME) ---
def style_internal(val):
    if not isinstance(val, (int, float)) or val == 0: return 'color: rgba(128,128,128,0.3)'
    if val < 8: return 'color: #fca5a5' # Rosso chiaro
    if val <= 10: return 'color: #93c5fd' # Blu chiaro
    return 'color: #4ade80; font-weight: bold' # Verde acceso

def style_external(val):
    if not isinstance(val, (int, float)) or val == 0: return 'color: rgba(128,128,128,0.3)'
    if val < 8: return 'color: #fdba74' # Arancio chiaro
    if val <= 9: return 'color: #fbbf24' # Giallo/Ambra
    return 'color: #f97316; font-weight: bold' # Arancio acceso
...
# ==============================================================================
# ★ TAB 4: AUDIT UNIFICATO (DARK MODE READY) ★
# ==============================================================================
with tab_audit:
    st.markdown("## ⚖️ Control Room Finanziaria (Full Costing)")
    st.info("ℹ️ Analisi con costi occulti (TFR, 13ma, DPI, Noli) inclusi.")
    ...
    with c_left:
        ...
        # HTML TABLE - DARK MODE COMPATIBLE
        # Usiamo classi CSS definite all'inizio, niente stili inline con colori fissi
        html_tab = f"""
        <table class="audit-table">
            <thead>
                <tr>
                    <th width="40%">VOCE</th>
                    <th width="35%" style="text-align:right;">DETTAGLIO</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-muted" style="text-align:right;">(€{net_op}x{h_pay}h)+€{one_op}</td>
                    <td class="text-red" style="text-align:right;">€ {costo_pers_op_gg*n_op:,.2f}</td>
                </tr>
                <tr>
                    <td>(-) Manodopera Capi ({n_cap})</td>
                    <td class="text-muted" style="text-align:right;">(€{net_cap}x{h_pay}h)+€{one_cap}</td>
                    <td class="text-red" style="text-align:right;">€ {costo_pers_cap_gg*n_cap:,.2f}</td>
                </tr>
                <tr>
                ...
                <tr style="border-top: 2px solid var(--text-color); font-size:1.1em;">
    ...
    with c_right:
        ...
        fig = go.Figure(go.Waterfall(
            ...
            x = ["Fatturato", "Manodopera", "Ticket/Noli", "Margine Ind.", "CHECK", "Spese Gen.", "Utile Netto"],
            y = [
                r_tot_sq_gg, 
                -(costo_labor_sq_gg), 
                -(spese_vive_tot_gg), 
            ...
            decreasing = {"marker":{"color":"#f87171"}}, # Rosso soft
            increasing = {"marker":{"color":"#4ade80"}}, # Verde soft
            totals = {"marker":{"color":"#60a5fa"}}      # Blu soft
        ))
        fig.update_layout(title="Analisi Squadra (1 Giorno)", showlegend=False, height=500, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font=dict(color="grey")) # Dark mode friendly
